...
я создал ветку Илим и в нем создал три коммита 
потом из ветки три сделал git merge two и из ветки один git rebase three 
    Ответы на вопросы:
     git checkout более универсален, но может сбивать с толку из-за множества функций. git switch специально разработан для переключения ветвей, что делает его более простым и понятным для этой цели
     2 . слияние двух веток
     3.комииты меняются и переписываются в одну
     4.можно и  git branch -a

 удалил ветки два и три     